...
def next_tokens(text):
     ...
     top_k_probabilities, top_k_indices = torch.topk(last_token_probabilities, 5)
...
def extract_error_info(error_information, inputCode):
     ...
     # match the first loc_end, because  re.search only find the first loc_end
     ...
     if os.path.exists(temp_file_path):          
          ...
          if error:
               ...
               if error_info:  # Check if error_info is not None
                    ...
                    elif error_type == "Syntaxerr.Unclosed":
                         return True,error_type+ "_continue"
...
directory_path= "/home/wangao/lab_work/checkml/syncor/log/syncor3"
...
def generate_ocaml_code(file_path,s_text):
    ...
    while S:
        ...
        if(result == False):
          ...
          continue
        if errtype == "noerr" and s.ifeos == True:
          current_time = time.time()
          process_time = current_time - start_time
          with open(file_path, "a") as file:
               file.write("<<<<<<<<<<<<< FINISH >>>>>>>>>>>>\n\n\n\n")
               file.write(f"----------END and No Syntax Error--------\n\nprocessing time : :{process_time}")
          return s.string
        elif s.ifeos == False:
          ...
          #for i,token in enumerate(t_tokens):
               ...
               st = s.string + token
               newst = tokenSet(eossign, st)
               S.append(newst)
               
               print_token(token, top_k_probabilities[i].item(),file_path)
